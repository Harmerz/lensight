...
export function StepsRegister({ items, current }) {
  return (
      ...
      {items.map((item, index) => (
        <div key={item.name + item.description} className="flex h-full w-full flex-row">
          <div className="min-h-16 flex w-fit flex-col items-start justify-start">
            <div
              className={`flex h-12 w-12 items-center justify-center rounded-full border text-center text-base font-bold  ${
            ...
            {index !== items.length - 1 && (
              <div className="my-2 flex w-full grow items-center justify-center">
                <div className="flex h-full min-h-[40px] w-[2px] items-center justify-center bg-neutral-50" />
              </div>
            )}
          </div>
